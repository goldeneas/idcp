...
#define HANDLE_DATAGRAM_BRANCH(field_name, envelope, envelope_struct, handler_function, sockaddr,   \
        context)                                                                                    \
    if (envelope->which_payload == envelope_struct##_##field_name##_tag) {                          \
        handler_function(&envelope->payload.field_name, sockaddr, context);                         \
        return;                                                                                     \
    }
